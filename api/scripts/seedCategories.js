...
async function seed() {
  ...
  await CategoryGroup.insertMany([
    { displayName: 'Central', code: 'central', scope: 'central', categories: categories.map(c => c._id) },
    { displayName: 'State', code: 'state', scope: 'state', categories: categories.map(c => c._id) },
    { displayName: 'None', code: 'none', scope: 'none', categories: categories.map(c => c._id) },
  ]);

  console.log('Seeded: 3 category groups (Central, State, None) with all categories');
  mongoose.disconnect();
}
